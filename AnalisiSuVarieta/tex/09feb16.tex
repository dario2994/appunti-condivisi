...
\begin{proposition} \label{prop:ProprietaMappeFibrati}
	...
	\begin{enumerate}
		\item $(g\circ f)_* = g_* \circ f_*$; \label{pmf:Distributiva}
...
\begin{itemize}
	\item Se $f \in C^\infty(M)$ e $t \in \tau_s^r(M)$, allora $ft \in \tau_s^r(M)$ è definita da $(ft)(p) \coloneqq f(p) t(p)$.
\end{itemize}
Se $X_i \in \chi(M)$, $\alpha^i \in \chi^*(M)$, $t \in \tau_s^r(M)$ e $t' \in \tau_{s'}^{r'}(M)$, possiamo definire
\begin{itemize}
	\item $t(\alpha^1,\ldots,\alpha^r,X_1,\ldots, X_s)\in C^\infty(M)$ tale che
	\begin{equation*}
		p \mapsto (t(p))(\alpha^1(p),\ldots,\alpha^r(p),X_1(q),\ldots, X_s(q))
	\end{equation*}
	
	\item $t\otimes t' \in \tau_{s+s'}^{r+r'}(M)$ tale che $p \mapsto t(p)\otimes t'(p)$.
\end{itemize}
