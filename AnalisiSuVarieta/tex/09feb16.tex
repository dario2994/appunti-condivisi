...
\begin{definition} \index{mappelocali!fibrati!tensoriali}
...
\begin{proof}
	...
	Per la \ref{aft:CambiCarta}, abbiamo che $\alpha \coloneqq \tilde\varphi_j\circ \tilde\varphi_i^{-1}$ è un isomorfismo locale di fibrati. Per la proposizione $\alpha_* = (\tilde\varphi_j)_*\circ (\tilde\varphi_i)_*^{-1}$ è anche un isomorfismo locale di fibrati. %TODO: aggiungere cref
\end{proof}
...
\begin{proposition} \label{prop:ProprietaMappeFibrati} %TODO: cambiare label
	Siano $f:E\to E'$ e $g:E' \to E''$ mappe tra fibrati che sono isomorfismi su ogni fibra. Allora
	\begin{enumerate}
		\item $g\circ f$ è ancora un isomorfismo su ogni fibra e $(g\circ f)_* = g_* \circ f_*$; \label{pmf:Distributiva}
		\item se $i:E \to E$ è l'identità, allora $i_* : T_s^r(E) \to T_s^r(E)$ è l'identità; \label{pmf:Identita}
		\item se $f:E \to E'$ è un isomorfismo tra fibrati, lo è anche $f^{-1}$ e $(f_*)^{-1} = (f^{-1})_*$. \label{pmf:Isomorfismo}
	\end{enumerate}
\end{proposition}
\begin{proof}
	Per la \ref{pmf:Distributiva} basta verificare la proprietà sui rappresentanti locali. La \ref{pmf:Identita} è ovvia e insieme alla \ref{pmf:Distributiva} implica la \ref{pmf:Isomorfismo}.
\end{proof}
...
Vediamo ora alcune operazioni.

\begin{itemize}
	\item Se $f \in C^\infty(M)$ e $t \in \Tau_s^r(M)$, allora $ft \in \Tau_s^r(M)$ è definita da $(ft)(p) \coloneqq f(p) t(p)$.
\end{itemize}
Se $X_i \in \chi(M)$, $\alpha^i \in \chi^*(M)$, $t \in \Tau_s^r(M)$ e $t' \in \Tau_{s'}^{r'}(M)$, possiamo definire
\begin{itemize}
