...
\section{Definizioni} %TODO: cambiare titolo nell'ottica di un'unione di capitoli (distribuzioni e varietà integrali)
...
\begin{definition} \index{distribuzione!unidimensionale}
	Una \emph{distribuzione unidimensionale} $\Delta$ in un aperto $U$ di $M$ è una scelta regolare di un sottospazio 1-dimensionale $\Delta_p$ di $T_pM$ per ogni $p\in U$.
\end{definition}
...
\begin{example} %TODO: sistemare
	...
	Se la velocità di una curva $\gamma$ sta nel tangente, allora $\frac\de{\de s}g(\gamma(s))=0$ (perché $T_pN=\{v\ : \ \Scal{\grad g(p)}{v} =0\})$.}. %TODO: perché per s abbastanza piccolo? a me pare per tutti gli s
...
\begin{proof}
	...
	Allora, se $X=\sum_{j=1}^k\alpha_j\DerParz{}{y^j}$ con $\alpha_j\in C^\infty$ e $\bar X = \sum_{j=1}^n\beta^j\DerParz{}{x^j}$, allora per linearità di $Ti$ abbiamo $\beta^j=\alpha^j$ per $j=1,\ldots,k$. Quindi $\beta^j$ è regolare in $x$ e $\bar X\in\chi(N)$.
\end{proof}
...
Notiamo che $Ti[\bar X,\bar Y](p) = [Ti\bar X,Ti\bar Y](i(p)) = [X,Y] (i(p))$. Per ipotesi, dato $v\in T_pN$, allora $(Ti)v\in\Delta_{i(p)}$; di conseguenza $[X,Y](i(p))$ deve appartenere a $\Delta_{i(p)}$ per ogni $p\in N$. %TODO: scrivere meglio il fatto che la parentesi di Lie passa a sottovarietà
