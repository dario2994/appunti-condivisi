...
\begin{definition}
	Data $M$ varietà con bordo, definiamo $\Int M = \bigcup_{i} \varphi_i^{-1}(\Int (\varphi_i(U_i)))$ e $\partial M = \bigcup_i \varphi_i^{-1}(\partial (\varphi_i(U_i)))$ come parte interna e bordo di $M$.
\end{definition}
...
\begin{theorem} [Gauss]\label{Teo:Gauss}
...
\end{corollary}
\begin{proof}
	Localmente, scegliamo una carta $(U,\varphi)$ tale che $n_{\partial M} = - \DerParz{}{x^m}$. Se $\seqb v{m-1},$ è una base orientata di $T_x\partial M$, vale:
	
	$\mu_{\partial M}(x) (\seqb v{m-1},) = \mu_m(x) (-\DerParz{}{x^m},\seqb v{m-1},)$. Segue allora che:
	
	\begin{align*}
		(i_X\mu_M)(x)(\seqb v{m-1},) &= \mu_M (X^i(x)v_i + X^m\DerParz{}{x^m},\seqb v{m-1},) \\
		&= X^m(x) \mu_{\partial M}(x) (\seqb v{m-1},) \punto
	\end{align*}
	Inoltre $X^m = -g(X,n_{\partial M})$, quindi basta applicare il \cref{Teo:Gauss}.
...
Consideriamo una prima applicazione per calcolare i volumi delle sfere.
...
Definisco $\cos\theta = \abs{p-p'} = (1-t^2)^{\frac 12}$ con $p'$ la proiezione su $\{t\}\times \{0\}$. Allora $\abs{\grad \pi(p)} = (1-t^2)^{\frac 12}$.
