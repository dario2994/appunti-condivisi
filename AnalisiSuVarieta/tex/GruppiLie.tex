...
\begin{proof}
	...
	Sia ora $b\in G$, allora $bKb^{-1}$ è connesso ed $e\in bKb^{-1}$, quindi $bKb^{-1}\subseteq K$ per ogni $b\in G$ e perciò6 $K$ è normale. Inoltre $K$ è chiuso essendo una componente connessa.
...
\begin{proof}
	...
	Sia $H$ una varietà integrale massimale contenente $e$, la cui esistenza ci à garantita dal \cref{thm:Frobenius}. Se $b \in G$, allora $(L_b)_* (\Delta_a) = \Delta_{ba}$. Quindi $(L_b)_*$ lascia invariante la distribuzione. Perciò $L_b$ permuta le varietà integrali massimali di $\Delta$.
