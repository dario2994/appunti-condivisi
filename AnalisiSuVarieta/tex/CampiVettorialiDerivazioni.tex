...
\begin{exercise}
	Sia $X$ campo vettoriale su $\R^n$ di classe $C^r$ e sia $f:\R^n\to \R$ di classe $C^1$ e propria (cioè controimmagine di compatti è compatta). Supponiamo che esistano $K,L>0$ tali che $\abs{X(f)(p)}\le K\abs{f(p)}+L$ per ogni $p\in\R^n$ \footnote{Per definizione $X(f)(p)=\Diff_{X(p)}f(p)$.}. Dimostrare che $X$ è completo. %TODO: cosa vuol dire X(f)? È giusto? Prima c'era scritto X(f) = \Diff_X (f)
\end{exercise}
