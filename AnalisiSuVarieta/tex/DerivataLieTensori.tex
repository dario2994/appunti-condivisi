...
\begin{definition} \index{operatore!differenziale} \label{def:OperatoreDifferenziale} %TODO: aggiungere la proprietà D(fg)=D(f)g+fD(g)
...
\begin{theorem} \label{thm:EsistenzaOperatoriDifferenziali} %TODO: uniformare fra \otimes, \cdot e prodotto sottinteso
...
\begin{proof}
	...
	\begin{align*} %TODO: controllare se in altri posti si usava - - per andare a capo nelle formule
...
\begin{remark}
	Sia $\varphi_s$ il flusso generato da $X\in\chi(M)$. Allora, se $t\in\Tau_s^r(M)$, vale $\Lie_Xt = \frac{\de}{\de s} (\varphi_s^* t)$ \footnote{Ricordiamo che $\varphi_s$ è un diffeomorfismo e quindi è legale fare pull-back di tensori di ogni tipo.}. %TODO: la lettera s ha 2 significati, decidere se sistemare
\end{remark}