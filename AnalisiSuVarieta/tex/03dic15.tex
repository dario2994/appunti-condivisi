\chapter{Lezione 3 dicembre 2015}

\section{Distribuzioni e varietà integrali}

\begin{definition}
	Una distribuzione $k$-dimensionale in $U\subseteq M^n$ aperto ($k,n\in\N$, $k\le n$) è una mappa regolare tale che $p\mapsto \Delta_p$ sottospazio $k$-dimensionale di $T_pM$.
	
	In questo caso, mappa regolare significa che esistono $X_1,\ldots,X_k\in \chi(U)$ tali che per ogni $q\in U$ $X_i(q)\ne 0$ e tale che $\Delta_q=span\{X_1(q),\ldots,X_k(q)\}$ per ogni $q \in U$.
\end{definition}

\begin{definition}
	Una \emph{varietà integrale} per $\Delta$ è una sottovarietà $N$ di $M$ tale che, se $i:N\to M$, allora $(Ti)(T_pN)=\Delta_p$ per ogni $p\in N$.
\end{definition}

\begin{example}
	Se $k=1$ e se $X\in\chi(U)$, $X(p)\ne 0$ per $p\in U$. $\Delta_q=span \{X(q)\}$, $q\in\tilde U\subset\subset U$ è una distribuzione 1-dimensionale.
	
	Curve integrali di $X$ sono varietà integrali di $\Delta$.
\end{example}

Invece per $k>1$ le distribuzioni $k$-dimenzionali non sempre ammettono varietà integrali (tranne per $k=n$).

\begin{example}
	Sia $M=\R^3$ e $k=2$ e consideriamo la distribuzione $\Delta_p=span\{\DerParz{}{x}(p)+y\DerParz{}{z}(p),\DerParz{}{y}(p)\}$.
	
	Se $p_0=(x_0,y_0,z_0)$, $\Delta_{p_0}$ è il piano di equazione $\{z-z_0=y_0(x-x_0)\}$.
	
	Vogliamo mostrare che per questa distribuzione non esista una varietà integrale.
	...
	Se la velocità di una curva $\gamma$ sta nel tangente, allora $\frac\de{\de s}g(\gamma(s))=0$ (perché $T_pN=\{v\ : \ scalar(\grad g(p),v)=0\}$.}.
...
Ora vogliamo studiare quando una distribuzione proviene da una varietà integrale.

\subsection{Condizioni necessarie per l'esistenza di varietà integrali}
...
Sia $N$ varietà integrale per $\Delta$ e sia $i:N \to M$ l'inclusione.
...
\begin{lemma}\label{lemma:RestrizioneCampoVettoriale}
	Sia $i:N\to M$ un'immersione e sia $X\in\chi(M)$ tale che $X(i(p))\in (Ti)(T_pN)$. Allora esiste $\bar X\in\chi(N)$ tale che $(Ti)\bar X (p) = X(i(p))$.
\end{lemma}
...
\begin{proof}
	...
	È facile vedere che esistono carte $(V,\psi)$ e $(U,\varphi)$ in $N$ ed $M$ tali che $\varphi\circ i \circ\psi^{-1}(a^1,\ldots, a^k) = (a^1,\ldots,a^k,0,\ldots,0)$. Questo mostra che $(Ti)(\DerParz{}{x^j}\restrict{p}) = \DerParz{}{y^j}\restrict{i(p)}$ per $j=1,\ldots,k$.
...
\begin{proof}
	...
	Per Leibniz e poiché $[X,Y] = \Lie_XY$, vale che \begin{equation*}
		[a^iX_i,b^jY_j] = a^ib^j[X_i,X_j]+a^i(X_ib^j)Y_j+b^j(Y_ja^i)X_i\virgola %TODO: questa formula ha dei problemi
	\end{equation*}
	dove la parte a destra appartiene a $\Delta$.
\end{proof}

\subsection{Condizione sufficiente per l'esistenza di varietà integrali}

\begin{theorem}[Frobenius]
	Sia $\Delta$ una distribuzione (regolare) $k$-dimensionale in $M$. Allora, se $\Delta$ è integrabile, per ogni $p\in M$ esiste una carta $(U,\varphi)$ tale che $\varphi(p) = 0\in\R^n$, $\varphi(U)=\oo{-\varepsilon}{\varepsilon}^n$ e tale che per ogni $a^{k+1},\ldots,a^n$ con $\abs{a^j}<\varepsilon$ l'insieme $\{q\in U\ : \ x^j(q)=a^j, j=k+1,\ldots,n\}$ è una varietà integrale di $\Delta$.
	
	Inoltre localmente vicino a $p$ ogni varietà integrale di $\Delta$ è di questo tipo.
\end{theorem}

\begin{proof}
	Possiamo supporre $M=\R^n$ e $\Delta_0=span\{\DerParz{}{t^1}(0),\ldots,\DerParz{}{t^k}(0)\}$. Sia $\pi:\R^n\to\R^k$ la proiezione sulle prime $k$ coordinate. Allora $\T\pi\restrict{\Delta_0}:\Delta_0\to \T_0\R^k$ è un isomorfismo.
	Per continuità $\T_\pi\restrict{\Delta_q}:\Delta_q\in\R^k$ è un isomorfismo per $q$ vicino a 0.
	...
	Poiché $\T\pi$ è un isomorfismo, $\comm{X_i}{X_j}(q)=0$ per ogni $q$ vicino a 0.
	...
	Sia ora $N$ una varietà integrale connessa di $\Delta$ vicino a 0 e sia $i:N\in U$ la sua inclusione. Consideriamo (nelle coordinate $(x^i)$ definite sopra) la funzione $x^m\circ i:N\to \R$ per $m=k+1,\ldots,n$. Per ogni $v\in T_qN$ vale $v(x^m\circ i) = (\T i)(v)(x^m)=0$, perché $\Delta_q$ è generata da $\DerParz{}{x^i}(q)$ per $i=1,\ldots,k$. Quindi, dato che $N$ è connessa, $x^m$ è costante su $N$ per $m=k+1,\ldots,n$.
\end{proof}
