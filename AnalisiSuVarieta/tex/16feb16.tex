...
\begin{remark}
	...
	\begin{equation*}
	\Lin_{C^\infty(M)}^{r+s}(\chi^*(M),\ldots,\chi^*(M),\chi(M),\ldots,\chi(M),C^\infty(M))\punto %TODO: r volte ed s volte
	\end{equation*}
\end{remark}
...
\begin{proof}
	...
	\begin{align*}
		...
		&= f((\mathcal D\alpha) X)
	\end{align*}
	[utilizzato le proprietà 3,1] %TODO: sistemare
...
\begin{proof}
	...
	\varphi^*\Lie_{\varphi_*X}((\varphi_*t)(\varphi_*\alpha_1,\ldots,\varphi_*\alpha_r,\varphi_*X_1,\ldots,\varphi_*X_s))) =	$ %TODO: aiuto!!!! mi sono arresa
...
$(\Lie_{X'}\alpha') v = (\Diff \alpha'X') v + \alpha (\Diff X', v)$ %TODO: non ci sto più capendo un cazzo
