...
\subsection{Massa variabile nel tempo}
In un problema con un corpo di massa variabile, dette $-\de m$ la massa espulsa (o acquisita) in un tempo $\de t$ e $\vec v_{rel}$ la velocità della massa espulsa (o acquisita) relativa al corpo, vale la seguente relazione:
\begin{equation}\label{EqMassaVariabile}
	\vec F=\frac{\de}{\de t} (m\vec v)-\frac{\de m}{\de t}(\vec v+\vec{v_{rel}})
\end{equation}
dove $\vec F$ è la risultante delle forze esterne agenti sul sistema del corpo e della massa espulsa (o acquisita).
\end{document}
