...
\begin{definition}\label{CompletezzaMisura}
	Dato $(X,\mathcal A,\mu)$ uno spazio di misura, la misura $\mu$ si dice completa se per ogni $A\in\mathcal A$ tale che $\mu(A)=0$, ogni suo sottinsieme è anch'esso appartenente ad $\mathcal A$.
\end{definition}
...
\end{proposition}
\begin{proof}TODO: dimostrazione
	Dimostro intanto che $\mathcal A^*$ è una \sigalg{}.
	Dato $A^*\in\mathcal A^*$, esistono per definizione $A\in\mathcal A$ e $N\subseteq B\in\mathcal A$ dove $\mu(B)=0$. Posso facilmente imporre $B\cap A^*=\emptyset$, visto che se così non è posso ridefinire $N,B$ facendone la differenza con $A^*$. Assumo perciò $A^*,B$ disgiunti.
	Chiamando $M=B\setminus N$, vale facilmente:
	\begin{equation*}
		(A^*)^c=(A\cup N)^c=A^c\cap N^c=(A^c\cap B^c )\cup M \in\mathcal A^*
	\end{equation*}
	dove l'ultima appartenenza vale poichè $M$ è anch'esso sottinsieme di un trascurabile. Quindi $\mathcal A^*$ è stabile per passaggio al complementare.
	
	Ora considero $(A^*_n)_{n\in\mathbb N}$ una successione di insiemi, e scelgo $(A_n)_{n\in\mathbb N},(B_n)_{n\in\mathbb N},(N_n)_{n\in\mathbb N}$ tali che valgano $A^*_n=A_n\cup N_n$ e $N_n\subseteq B_n\in\mathcal A$ con $\mu(B_n)=0$.
	...
	\end{align*}
	dove l'ultima uguaglianza dipende dal fatto che la misura è \sigsubadd{}, TODO: dimostrare Subaddittività
\end{proof}
