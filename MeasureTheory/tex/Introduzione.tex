...
\begin{proof}
	...
	Per l'unione, consideriamo $A,B\in\S$. Poiché vale $A\cup B=(A\setminus B)\sqcup(A\cap B)\sqcup(B\setminus A)$, viste le proprietà di un \semiring{}, risulta $A\cup B\in \sqcup\S$. Da questo è facile ottenere che anche unioni finite di elementi di $\S$ appartengono a $\sqcup\S$.
...
\begin{definition}[{\sigadd[ità]}]
	...
	\begin{equation*}
		\mu\left(\bigsqcup_{n\in\N}F_n \right)=\sum_{n\in\N} \mu(F_n) \punto
	\end{equation*}
\end{definition}
...
\begin{proof}
	...
	otteniamo $\mu(B)=\mu(B\setminus A)+\mu(A)\ge\mu(A)$ che è la tesi.
\end{proof}
...
\begin{remark}\label{nota:UnioneTrascurabili}
	Unione numerabile di insiemi trascurabili è a sua volta trascurabile.
\end{remark}
\begin{proof}
...
\begin{definition}\label{def:QuasiOvunque}
	Nel seguito i termini \emph{quasi ovunque} e \emph{quasi sempre} e \emph{quasi per ogni}, implicitamente riferiti ad uno spazio di misura, definiranno proposizioni che valgono per ogni elemento tranne al più per un sottoinsieme trascurabile di elementi.
\end{definition}
...
\begin{proof}
	...
	\begin{align*}
		\mu(A)\le \mu(A\cup Q) \le\mu(A)+\mu(Q)=\mu(A) &\implies \mu(A)=\mu(A\cup Q) \puntovirgola\\
		\mu(A')\le \mu(A'\cup Q) \le\mu(A')+\mu(Q)=\mu(A') &\implies \mu(A')=\mu(A'\cup Q) \virgola
	\end{align*}
	...
	\begin{equation*}
		\sum_{n\in\N} \tilde\mu(A^*_n)=\sum_{n\in\N} \mu(A_n)=\mu\left(\bigcup_{n\in\N} A_n\right)=\tilde\mu\left(\bigcup_{n\in\N} A^*_n\right)\virgola
	\end{equation*}
...
\begin{proposition}\label{prop:LimiteMonotonoCrescenteMisura}
	...
	\begin{equation*}
		\mu\left(\bigcup_{n\in\N} A_n\right)=\lim_{n\to\infty} \mu(A_n) \punto
	\end{equation*}
\end{proposition}
\begin{proof}
...
\begin{corollary}\label{cor:LimiteMonotonoDecrescenteMisura}
	...
	\begin{equation*}
		\mu\left(\bigcap_{n\in\N} A_n\right)=\lim_{n\to\infty} \mu(A_n) \punto
	\end{equation*}
\end{corollary}
\begin{proof}
	...
	\begin{equation}\label{eq:ContoLimiteMonotonoMisura}
		\mu\left(A_0\setminus\bigcap_{n\in\N}A_n\right)=\mu\left(\bigcup_{n\in\N}B_n\right)=\lim_{n\to\infty}\mu(B_n)
		=\lim_{n\to\infty}\mu(A_0\setminus A_n) \punto
	\end{equation}
...
\begin{definition}
	Una terna $(X,\S,\mu)$ tale che $\S\subseteq\mathcal P(X)$ è un \semiring{} e $\mu:\S\to \Rpiu$ è \sigadd{} con $\mu(\emptyset)=0$, la chiamiamo spazio di misura elementare e la funzione $\mu$ viene detta misura elementare o premisura.
\end{definition}
...
\begin{proof}
	...
	\begin{gather*}
		...
		\Downarrow\\
		\mu(A_n)=\sum_{i,j\in\N}\mu(F^{ni}_j)+\sum_{i\in\N}\mu(E^n_i) \puntovirgola\\
		B_n=\bigsqcup_{i\in\N}C_{in}=\bigsqcup_{i,j\in\N}F^{in}_j
		\implies  \mu(B_n)=\sum_{i,j\in\N}\mu(F^{in}_j) \virgola
	\end{gather*}
	quindi sommando su $n\in\N$ arriviamo alle disuguaglianze
	\begin{align*}
