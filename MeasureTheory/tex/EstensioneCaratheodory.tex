...
\begin{proof}
	...
	\begin{description}
		\item[\ImplicationProof{MisurabileEquivalenze}{UnioniDaFuoriEquivalenze}] Sia $A$ misurabile e $(X_n)_{n\in\N}\subseteq \S$ una partizione numerabile di $X$ tale che $\mu(A_n)<+\infty$ (questa esiste poichè $\mu$ è \sigfin{}).
		...
		\begin{equation*}
			\mu^*(A_n)\le \mu^*(S_n) \le \mu^*(A_n)+\frac\epsilon{2^n}\Longrightarrow
		...
		\begin{equation}\label{DifficileFreccia12Misurabili}
			0\le \mu'(S_n\setminus A_n) \le \frac\epsilon{2^n} \Longrightarrow \mu^*(S_n\setminus A_n)\le \frac\epsilon{2^n}
		\end{equation}
		...
		\begin{equation*}
			\forall n\in\N:\ \left(B\subseteq B_n\to B\setminus A\subseteq B_n\setminus A\to \mu^*(B\setminus A)\le \mu^*(B_n\setminus A)\le \frac1n\right) \Longrightarrow \mu^*(B\setminus A)=0
		\end{equation*}
		e perciò $B$ rispetta tutte le richieste e dimostra l'implicazione cercata.
