...
\begin{lemma}\label{lemma:CoperturaPalle}
	Dato $S\in\S_n$, per ogni $\epsilon>0$, esiste una famiglia finita di palle $B_1,\dots,B_k$ tali che la loro unione contiene $S$ ed inoltre rispettano la disuguaglianza
	\begin{equation*}
		\sum_{i=1}^k m_n(B_i)\le (1+\epsilon)m_n(S)\punto
	\end{equation*}
\end{lemma}
\begin{proof}
	Senza perdità di generalità possiamo assumere $S=\co{0}{l_1}\times\cdots\times\co{0}{l_n}$ e sia $B$ una palla contenuta in $S$ con centro $c$ e raggio $r$, cioè $B=B_r(x)$. Sia $u=m_n(B)$.
	
	L'idea è che se realizziamo un ricoprimento con palle la cui somma delle misure sia alpiù $m_n(S)(1+\epsilon)$ allora, attraverso un procedimento ``frattale'', riusciamo a costruire un ricoprimento con palle la cui misura totale sia $m_n(S)\left(1+\epsilon(1-u)+\epsilon'\right)$, dove $\epsilon'$ è un numero positivo arbitrariamente piccolo. Se mostriamo questo è evidente che riusciamo a costruire ricoprimenti con palle che realizzano la richiesta dell'enunciato.
	Assumiamo allora di avere un ricoprimento che realizza misura totale $m_n(S)(1+\epsilon)$.
	
	Fissato $\delta>0$, definiamo $B_{\delta}=B_{r+\delta}(x)\setminus B_{r-\delta}(x)$ e notiamo che, scegliendo $\delta$ piccolo a sufficienza, la quantità  $m_n(S_\delta)$ diviene piccola a piacere in virtù dell'omogeneità della misura di Lebesgue.
	
	Fissato un intero $k\in\N$, chiamiamo $P:\Z^n\to\S_n$ la funzione così definita:
	\begin{equation*}
		P(a_1,a_2,\dots,a_n)=\co{\frac{a_1l_1}k}{\frac{(a_1+1)l_1}k}\times\cdots\times\co{\frac{a_nl_n}k}{\frac{(a_n+1)l_n}k} \punto
	\end{equation*}
	Intuitivamente quello che faremo è \emph{ricopiare} il ricoprimento che realizza $m_n(S)(1+\epsilon)$ su tutti i $P(v)$ che intersecano $S\setminus B$.
	
	Chiamiamo $v\in\Z^n$ interessante se $P(v)\cap S\setminus B\not=\emptyset$. 
	Dalla definizione di $P(v)$ si ricava che se $v$ è interessante allora o $P(V)\subseteq S\setminus B$ oppure $P(V)\cap\partial B\not=\emptyset$, dove con $\partial B$ indichiamo l'ipersfera che forma il bordo di $B$. Nel primo di questi casi chiamiamo $v$ \emph{interno}, altrimenti lo definiamo \emph{confinante}.
	
	%TODO: Da finire la dimostrazione!
\end{proof}
...
\begin{proof}
	...
	\end{align*}
	Quest'ultima equazione è vera per ogni $\epsilon>0$ e allora, per la definizione di misura esterna, ne segue
