...
\begin{proof}
	...
	Fissiamo arbitrariamente una palla $B$ contenuta in $S$ con centro $c$ e raggio $r$, cioè $B=B_r(c)$. Per comodità denotiamo $\alpha=m_n(B)$.
	...
	L'idea è, dato un ricoprimento con palle la cui \emph{misura totale} sia al più $1+\epsilon$, costruire, attraverso un procedimento ``frattale'', un ricoprimento con palle la cui \emph{misura totale} sia $1+\epsilon(1-\alpha)+\epsilon'$, dove $\epsilon'$ è un numero positivo arbitrariamente piccolo. Se mostriamo ciò, risulta evidente che riusciamo a costruire ricoprimenti con palle che realizzano la richiesta dell'enunciato.
	Assumiamo allora di avere un ricoprimento che realizza la \emph{misura totale} $1+\epsilon$.
	
	Fissato $\delta>0$, definiamo $B_{\delta}=B_{r+\delta}(c)\setminus B_{r-\delta}(c)$ e notiamo che, scegliendo $\delta$ piccolo a sufficienza, la quantità  $m_n(B_\delta)$ diviene piccola a piacere in virtù dell'omogeneità della misura di Lebesgue. Fondamentalmente $B_\delta$ è un ingrassamento, piccolo a piacere in funzione di $\delta$, del bordo di $B$.
	...
	Grazie alla \cref{nota:LebesgueProprieta}, risulta chiaro che ognuno dei $P_k(v)$ può essere coperto con palle moltiplicando al più per $1+\epsilon$ la sua area (semplicemente riscalando e traslando il ricoprimento di $S$) e unendo questo all'\cref{eq:CoperturaPalle} arriviamo a trovare un ricoprimento con palle di $S\setminus B$ con \emph{misura totale} minore o uguale a
	\begin{equation*}
		(1+\epsilon)\left(m_n(S\setminus B)+m_n(B_\delta)\right) \punto
	\end{equation*}
	Se a tale ricoprimento con palle aggiungiamo anche la palla $B$ ricopriamo interamente $S$ e realizziamo una \emph{misura totale} minore o uguale a
	\begin{equation*}
		(1+\epsilon)\left(m_n(S\setminus B)+m_n(B_\delta)\right)+\alpha
		=1+\epsilon(1-\alpha)+
...
\begin{proof}
	...
	In virtù della lipschitzianità di $f$ e dell'omogeneità della misura di Lebesgue mostrata nella \cref{nota:LebesgueProprieta}, se $B$ è una palla qualunque, risulta evidente che 
	\begin{equation*}
