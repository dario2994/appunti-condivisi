...
\section{Misura di Lebesgue}\label{sezione:MisuraLebesgue}
...
\begin{proof}
	...
	In modo del tutto analogo a come abbiamo definito $F_{\lambda}$\footnote{Tutto è uguale, solo che gli intervalli vanno rimpiccioliti \emph{dalla parte aperta}.}, costruiamo $S'\in\S_n$ tale che la sua chiusura sia contenuta in $S$ e valga la disuguaglianza $m_n(S')\ge m_n(S)-\epsilon$.
	
	Grazie alla \cref{nota:ParteInternaAllargamento} abbiamo che la successione composta dalle parti interne degli elementi della successione $\left(F_{\frac\epsilon{2^i}}(S_i)\right)_{i\in\N}$ è un ricoprimento aperto di $S$ e quindi, per quanto detto, della chiusura di $S'$.
	Perciò, per la compattezza della chiusura di $S'$, esiste un insieme finito di indici $I\subseteq \N$ tale che $\left(F_{\frac\epsilon{2^i}}(S_i)\right)_{i\in I}$ è un ricoprimento finito di $S'$.
	...
	\begin{multline*}
		...
		\ge -\epsilon+\sum_{i\in I}m_n\left(F_{\frac\epsilon{2^i}}(S_i)\right)\ge m_n(S')-\epsilon \ge m_n(S)-2\epsilon
	\end{multline*}
	e visto che questa vale per ogni $\epsilon>0$ ne ricaviamo
	\begin{equation*}
...
\begin{proof}
	...
	È facile vedere che l'unione di questi, che è ovviamente numerabile visto che $\mathbb Q^{n+1}$ lo è, è tutto l'aperto.
...
\begin{proof}
	...
	Unendo quanto detto e sfruttando la \sigsubadd[ità] della misura esterna si ottiene proprio $m_n^*(C\setminus A)<2\epsilon$ e questo conclude la dimostrazione di \ImplicationProof{it:LEMMisurabile}{it:LEMApertoFuori}.
...
\begin{lemma}\label{lemma:CoperturaPalle}
	Dato $S\in\S_n$, per ogni $\epsilon>0$, esiste una famiglia finita di palle aperte $B_1,\dots,B_k$ tali che la loro unione contiene $S$ ed inoltre rispettano la disuguaglianza
	\begin{equation*}
		\sum_{i=1}^k m_n(B_i)\le (1+\epsilon)m_n(S)\punto
	\end{equation*}
\end{lemma}
\begin{proof}
	Grazie alla \cref{nota:LebesgueProprieta} possiamo assumere senza perdità di generalità che $S=\co{0}{l_1}\times\cdots\times\co{0}{l_n}$ e che $m_n(S)=1$. 
	Fissiamo arbitrariamente una palla $B$ contenuta in $S$ con centro $c$ e raggio $r$, cioè $B=B_r(c)$. Per comodità denotiamo $A=m_n(B)$.
	Quando parleremo di \emph{misura totale} di un ricoprimento, ci riferiremo alla somma delle misure degli elementi del ricoprimento.
	
	L'idea è, dato un ricoprimento con palle la cui misura totale sia alpiù $1+\epsilon$, costruire, attraverso un procedimento ``frattale'', un ricoprimento con palle la cui misura totale sia $1+\epsilon(1-A)+\epsilon'$, dove $\epsilon'$ è un numero positivo arbitrariamente piccolo. Se mostriamo ciò, risulta evidente che riusciamo a costruire ricoprimenti con palle che realizzano la richiesta dell'enunciato.
	Assumiamo allora di avere un ricoprimento che realizza la misura totale $1+\epsilon$.
	
	Fissato $\delta>0$, definiamo $B_{\delta}=B_{r+\delta}(c)\setminus B_{r-\delta}(c)$ e notiamo che, scegliendo $\delta$ piccolo a sufficienza, la quantità  $m_n(S_\delta)$ diviene piccola a piacere in virtù dell'omogeneità della misura di Lebesgue. Fondamentalmente $B_\delta$ è un ingrassamento, piccolo a piacere in funzione di $\delta$, del bordo di $B$.
	
	Dato un intero $k\in\N$, chiamiamo $P_k:\Z^n\to\S_n$ la funzione così definita:
	\begin{equation*}
		P_k(a_1,a_2,\dots,a_n)=\co{\frac{a_1l_1}k}{\frac{(a_1+1)l_1}k}\times\cdots\times\co{\frac{a_nl_n}k}{\frac{(a_n+1)l_n}k} \punto
	\end{equation*}
	Intuitivamente quello che faremo sarà \emph{ricopiare} il ricoprimento che realizza $1+\epsilon$ su tutti i $P_k(v)$ che intersecano $S\setminus B$. Ciò è possibile perché il singolo $P_k(v)$ è un riscalamento traslato di $S$.
	
	Chiamiamo $v\in\Z^n$ \emph{interessante} se $P_k(v)\cap S\setminus B\not=\emptyset$. 
	Dalla definizione di $P_k(v)$ si ricava che se $v$ è \emph{interessante} allora o $P_k(v)\subseteq S\setminus B$ oppure $P_k(V)\cap\partial B\not=\emptyset$\footnote{In realtà possono esistere $v$ che rientrano in entrambi i casi, ma ciò non è un problema nel seguito della dimostrazione.}, dove con $\partial B$ indichiamo l'ipersfera che forma il bordo di $B$. Nel primo di questi casi chiamiamo $v$ \emph{interno}, altrimenti lo definiamo \emph{confinante}. Notiamo che l'unione dei $P_k(v)$ con $v$ \emph{interessante} contiene $S\setminus B$.
	
	Il punto cruciale è accorgersi che, pur di scegliere $k$ sufficientemente grande, se $v$ è \emph{confinante} allora $P_k(v)\subseteq B_\delta$. 
	Applicando quest'ultima osservazione e il fatto che se $u\not=v$ allora $P_k(u),P_k(v)$ sono disgiunti, si arriva alla seguente catena di disuguaglianze:
	\begin{equation}\label{eq:CoperturaPalle}
		\sum_{v\text{ \emph{interessante}}}m_n(P_k(v)) \le
		\sum_{v\text{ \emph{interno}}}m_n(P_k(v))+
		\sum_{v\text{ \emph{confinante}}}m_n(P_k(v))
		\le
		m_n(S\setminus B)+m_n(B_\delta) \punto
	\end{equation}
	
	Quindi con tante copie di $S$ scalate e traslate abbiamo ricoperto $S\setminus B$. 
	Grazie alla \cref{nota:LebesgueProprieta}, risulta chiaro che ognuno dei $P_k(v)$ può essere coperto con palle moltiplicando alpiù per $1+\epsilon$ la sua area (semplicemente riscalando e traslando il ricoprimento di $S$) e unendo questo all'\cref{eq:CoperturaPalle} arriviamo a trovare un ricoprimento con palle di $S\setminus B$ con misura totale minore o uguale a
	\begin{equation*}
		(1+\epsilon)\left(m_n(S\setminus B)+m_n(B_\delta)\right) \punto
	\end{equation*}
	Se a tale ricoprimento con palle aggiungiamo anche la palla $B$ ricopriamo interamente $S$ e realizziamo una misura totale minore o uguale a
	\begin{equation*}
		(1+\epsilon)\left(m_n(S\setminus B)+m_n(B_\delta)\right)+A
		=1+\epsilon(1-A)+
		(1+\epsilon)m_n(B_\delta)
	\end{equation*}
	e visto che $m_n(B_\delta)$ può essere scelto arbitrariamente piccolo, ciò coincide con la tesi.
\end{proof}
...
\begin{proposition}\label{prop:LipschitzTengonoMisurabili}
	...
	\begin{equation*}
		\forall A\in\M_n, A\subseteq E:\ m_n(f(A)) \le \lambda^n m_n(A) \punto
	\end{equation*}
\end{proposition}
\begin{proof}
	Dimostriamo innanzitutto la stima richiesta nel caso della misura esterna piuttosto che della misura.
	
	Fissiamo un ricoprimento $(S_i)_{i\in\N}\subseteq \S_n$ di un insieme $A\subseteq E$ misurabile ed un $\epsilon>0$.
	Il \cref{lemma:CoperturaPalle} ci assicura che per ogni $S_i$ esiste una famiglia finita di palle $(B^i_j)_{1\le j\le k_i}$ che lo ricopre e tale che $\sum_{j=1}^{k_i}m_n(B^i_j)\le(1+\epsilon)m_n(S_i)$.
	
	In virtù della lipschitzianità di $f$ e dell'omogeneità della misura di Lebesque mostrata nella \cref{nota:LebesgueProprieta}, se $B$ è una palla qualunque, risulta evidente che 
	\begin{equation*}
		m_n^*(f(B))\le \lambda^n m_n(B)\virgola
	\end{equation*}
	perciò, ricordando come sono state scelte le famiglie di palle $(B^i_j)$, è facile dedurre
	\begin{align*}
		m_n^*(f(A))= m_n^*\left(f\left(A\cap\bigcup_{\substack{i\in\N\\1\le j\le k_i}}B^i_j\right)\right)
		&\le \sum_{\substack{i\in\N\\1\le j\le k_i}} m_n^*\left(f\left( A\cap B^i_j\right)\right) \\
		&\le \sum_{\substack{i\in\N\\1\le j\le k_i}} \lambda^n m_n(B^i_j)\le \lambda^n(1+\epsilon)\sum_{i\in\N} m_n(S_i)\punto
	\end{align*}
	Essendo quest'ultima equazione vera per ogni $\epsilon>0$, per la definizione di misura esterna, ne segue
	\begin{equation}\label{eq:StimaMisuraEsternaLipschitz}
		m_n^*(f(A)) \le \lambda^n m_n^*(A) \virgola
	\end{equation}
...
\begin{proposition}\label{prop:BaseNumerabileLindelof}
...
\begin{proof}
	...
	Allora per quanto visto nella \cref{prop:LipschitzTengonoMisurabili} la funzione $O$ manda misurabili in misurabili e perciò ha senso definire la funzione di insiemi $m_n \circ O=\mu:\M_n\to\Rpiu$. 
...
\begin{proof}
	...
	Chiamiamo $C_i\subseteq\R^m$ l'ipercubo di lato $2i$ centrato nell'origine $C_i=\co{-i}{i}\times\cdots\times\co{-i}{i}$.
...
\begin{proof}
	...
	f(\alpha) = \left\{
		\begin{array}{ll}
			\tau & \quad \text{se $\alpha=0$} \virgola \\
			f(\beta)_{\delta}\cup f(\beta)_{\mathsf{c}} & \quad \text{se $\alpha=\beta+1$} \virgola\\
			\bigcup_{\beta < \alpha}f(\beta) & \quad \text{se $\alpha$ è un ordinale limite} \punto
		\end{array}
	\right.
	\]
	...
	quindi è successore di un ordinale $\gamma\ge \beta$. Per minimalità di $\alpha$, $f(\beta)\subseteq f(\gamma)$, tuttavia è sempre vero
	...
	Intanto, $|f(0)|=|\tau|=|\R|$\footnote{È fatto noto che gli aperti di $\R^n$ si scrivono come unione numerabile di palle di centro e raggio razionali e da questa osservazione discende che hanno la cardinalità del continuo.}. Poi, $|A_{\delta}|\le |A|^{|\N|}$ e $|A_{\mathsf{c}}|=|A|$.
	Infine $|\bigsqcup_{\beta<\alpha}E_{\beta}| \le |E||\alpha|$, con $|E_{\beta}|\le |E|$ per ogni $\beta < \alpha$. Tutto ciò porta a dire che
	\begin{equation}\label{eq:CardBorel}
		|\R|=|f(0)|\le |f(\omega_1)| \le |\omega_1||E| \le |E| \virgola
	\end{equation}
	purché $E$ sia un insieme di cardinalità maggiore od uguale a quella di tutti gli $f(\alpha)$ con $\alpha<\omega_1$;
	l'ultima disuguaglianza si ha perché è sicuramente vero che $|E|\ge |f(0)| = |\R| \ge |\omega_1|$.
	...
	\[
		|f(\alpha+1)|\le |f(\alpha)|+|f(\alpha)|^{\N} = |\R|^{|\N|} = |\R|
	\]
	e non può essere un ordinale limite
	\[
		|f(\alpha)|\le |\alpha| |\R| \le |\N||\R| = |\R| \punto
	\]
...
\begin{proof}
	...
	\[
		2m_n(C) = m_n(C)+m_n(C+x) \le m_n(C+B_{\rho}(0)) \le m_n(H)\virgola
	\]
