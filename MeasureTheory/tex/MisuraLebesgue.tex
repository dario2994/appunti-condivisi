...
Continueremo dimostrando le proprietà dei misurabili in questa nuova misura per poi caratterizzarla come l'unica misura che coincida con la nozione intutitiva di volume, cioè sia invariante per traslazione e sia normalizzata sul cubo unitario.

\begin{definition}\label{def:LebesgueMisura}
	Dato lo spazio $(\R_n,\S_n,m_n)$ che si è mostrato essere di misura elementare nel \cref{thm:LebesguePremisura}, sia $\M_n$, che verrà chiamato l'insieme dei misurabili secondo Lebesgue, la relativa \sigalg{} di Caratheodory e $m_n:\M_n\to\Rpiu$ \footnote{Qui abusiamo leggermente di notazione, visto che con $m_n$ si indicava la premisura.}, che verrà chiamata misura di Lebesgue, la misura associata, la cui esistenza ci è assicurata dal \cref{EstensioneCaratheodory}. 
...
\end{proposition}
\begin{proof}
	È una banale conseguenza di \cref{UnicitaCaratheodory}, che si può applicare ricordando la \cref{prop:LebesguePremisuraSigFin} e notando che la \sigalg{} generata da $\S_n$ sono i Boreliani come mostrato in \cref{prop:SigAlgUgualeBoreliani}.
\end{proof}
...
\begin{proof}
	...
	Consideriamo quindi la seguente funzione, definita $f:ON \rightarrow \mathcal{P}(\mathcal{P}(\R^n))$:
	\[
	f(\alpha) = \left\{
		\begin{array}{ll}
			\tau & \quad se\ \alpha=0, \\
...
\begin{proof}
	...
	Se assumiamo che $B$ sia misurabile, ora otteniamo un assurdo applicando \cref{eq:ContenimentiVitali} e ricordando che $m_n$ è invariante per traslazione come mostrato in \cref{nota:LebesgueProprieta}:
	\begin{equation*}
...
\end{theorem}
\begin{proof}
	Per il \cref{thm:LebesgueEquivalenzeMisurabilita}, fissato $\epsilon>0$, esistono $H\supseteq A$ aperto e $C\subseteq A$ chiuso tali che
	\[
		m_n(C)+\epsilon > m_n(A) > m_n(H)-\epsilon\punto
	\]
	Inoltre, dato che $C$ è chiuso e che $H^\mathsf{c}$ è chiuso e che $C \cap H^\mathsf{c} = \emptyset$, esiste $\rho>0$ tale che
	$C+B_{\rho}(0) \cap H^\mathsf{c} = \emptyset$ (basta prendere $\rho = \frac{1}{2}\inf\{|c-h|:c\in C\virgola\ h\in H\}$).
	Allora si ha che $\forall x\in B_{\rho}(0)$, $C \cap C+x \neq \emptyset$; altrimenti per monotonia della misura si ha
	\[
		2m_n(C) = m_n(C)+m_n(C+x) \leq m_n(C+B_{\rho}(0)) \leq m_n(H)\virgola
	\]
	ma questo contraddice la disuguaglianza precedente, pur di considerare $\epsilon< \frac{1}{2}m_n(C)$. E ciò è sempre possibile, dato che
	$m_n(A) > 0$ per ipotesi.
