...
\end{lemma}
\begin{proof}
	In virtù della \cref{nota:LebesgueProprieta} è sufficiente dimostrare l'enunciato nel caso particolare in cui $S=\co01^n$, infatti dato un elemento generico di $\S_n$ si può facilmente ricoprirlo, sprecando pochissima misura, con insiemi della forma $\co{a}{b}^n$ e poi coprire questi con palle esattamente come si coprirebbe l'insieme $\co01^n$.
	
	Ci riduciamo quindi al caso $S=\co01^n$ e chiamiamo $B=B_{\frac12}\left(\left(\frac12,\dots,\frac12\right)\right)$ e $u=m_n(B)$.
	L'idea è che se realizziamo $\epsilon$ allora, attraverso un procedimento ``frattale'', riusciamo a realizzare anche $\epsilon(1-u)+k$, dove $k$ è un numero arbitrariamente piccolo, e quindi è chiaro che riusciamo ad avere ricoprimenti con palle che ``sprecano'' misura piccola a piacere. %TODO: Da finire la dimostrazione!
\end{proof}
