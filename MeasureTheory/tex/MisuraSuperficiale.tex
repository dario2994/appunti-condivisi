...
\begin{exercise}
	Calcolare il volume di un tetraedro $n$-dimensionale di lato 1\footnote{Per tetraedro $n$-dimensionale di lato $d$ definiamo l'inviluppo convesso di $n+1$ punti in $\R^n$ a distanza $d$ uno dall'altro.}.
\end{exercise}
\begin{proof}
	Innanzitutto notiamo che un tetraedro $n$-dimensionale è misurabile in $\R^n$ in quanto è chiuso.
	Inoltre  dati due tetraedri $n$-dimensionali $T$ e $T'$ di lato rispettivamente $d$ e $d'$, per la \cref{nota:LebesgueInvarianteNormalizzazione}, vale che $m_n(T')=(\frac{d'}{d})^nm_n(T)$. Infatti possiamo ottenere un tetraedro dall'altro tramite una traslazione e un'omotetia.

	Consideriamo ora il piano affine $A=\{(x_1,x_2,\ldots,x_{n+1}) \mid x_1+x_2+\ldots +x_{n+1}=1\}$ in $\R^{n+1}$, in particolare consideriamo i punti $e_1,e_2,\ldots,e_{n+1}$ della base canonica di $\R^{n+1}$ su di esso e chiamiamo $T$ l'inviluppo convesso di questi $n+1$ punti. L'insieme $T$ si trova sul piano $A$ ed è l'inviluppo convesso di $n+1$ punti a distanza $\sqrt 2$ uno dall'altro, quindi per quanto detto sulla misura superficiale, $\sigma(T)$ equivale alla misura di un tetraedro $n$-dimensionale di lato $\sqrt 2$.
	...
	Per $m=1$ il poliedro è semplicemente il segmento di lato 1 in $\R$, quindi è verificato che $A_1=1$. Dimostriamo ora che se la tesi vale per $m=k$, allora vale per $m=k+1$. Se consideriamo il poliedro in $\R^{k+1}$ formato dallo zero e da tutti i vertici della base canonica tranne uno, otteniamo un poliedro che sta su un iperpiano di $\R^{k+1}$ e che equivale al poliedro nella dimensione precedente a meno dell'identificazione fra l'iperpiano e $\R^k$. Integrando sulla componente perpendicolare a tale piano otteniamo quindi
	\begin{equation*}
		A_{k+1}=\int_0^1t^kA_k\de x=\int_0^1t^k\frac 1{k!}\de x=\frac 1{(k+1)!}\virgola
	\end{equation*}
	poichè ad altezza $t$ la misura della sezione del poliedro è $t^kA_k$, in quanto tale sezione è un poliedro nella dimensione minore (infatti la misura è quella superficiale sull'iperpiano) di lato $t$.
