...
\begin{corollary}\label{cor:IntegrazionePerSeriePositive}
	Sia $(f_n)$ una successione di funzioni misurabili e positive, definite su uno spazio di misura $(X,\A,\mu)$ e a valori in $\Rbar$. Allora esiste $f:X\to\Rbar$ misurabili e positiva tale che $f(x)=\sum_{n=0}^\infty f_n(x)$, per ogni $x\in X$, e $\int f\de\mu=\sum_{n=0}^\infty\int f_n\de\mu$.
\end{corollary}
...
Dal \cref{lemma:L1NullaAlloraNulla} ricaviamo perciò che esistono funzioni $f$ non identicamente nulle tali che $\LNorm{f}=0$. Quindi $\LNorm{\cdot}$ non è una norma su $\L^1(X,\A,\mu)$ ma solo una seminorma (come già dimostrato nella \cref{prop:L1VettorialeConSeminorma}). Vediamo ora come ricondurci invece a considerare uno spazio vettoriale normato.
...
\begin{proof}
	...
	Se ora definiamo $f$ anche su $N$, assegnandogli dei valori arbitrari, abbiamo per il \cref{cor:IntegraleAMenoDiTrascurabili} che $f\in \L^1(X,\A,\mu)$ e che vale
	\begin{equation*}
...
\begin{remark}\label{nota:ConvergenzaL1IntegrazionePerSerie}
	Da notare che dalla dimostrazione, ricordando il \cref{cor:ConvergenzaL1Dominata}, segue anche che la convergenza di $s_n$ ad $f$ è in $\L^1(X,\A,\mu)$ ed è dominata, infatti $|s_n|\le g$.
\end{remark}
...
\begin{proposition}\label{prop:L1ImplicaSottosuccessioneQuasiOvunque}
	Sia $(f_n)$ una successione di funzioni integrabili convergenti ad $f$ nello spazio $\L^1(X,\A,\mu)$. Allora esiste una sottosuccessione $(f_{n_k})_{k\in\N}$ che converge quasi ovunque ad $f$ ed è dominata in $\L^1(X,\A,\mu)$.
\end{proposition}
...
\begin{proof}
	...
	Sia ora $h_k=f_{n_{k+1}}-f_{n_k}$ per ogni $k\in\N$. La successione $(h_k)$ rispetta le ipotesi del \cref{thm:IntegrazionePerSerie}, in quanto $\sum_{k=0}^\infty \LNorm {h_k}\le \sum_{k=0}^\infty 2^{-k}$, quindi la serie $\sum_{k=0}^\infty h_k$ converge quasi ovunque ed è dominata in $\L^1(X,\A,\mu)$.
...
\begin{remark}
	Notiamo che la tesi della \cref{prop:L1ImplicaSottosuccessioneQuasiOvunque} non può essere migliorata. Infatti in genere non è vero che se una successione di funzioni integrabili $(f_n)$ converge ad $f$ in $\L^1(X,\A,\mu)$, allora converge quasi ovunque ad $f$.
\end{remark}
\begin{proof}
	...
	È facile verificare che in $\L^1(X,\A,\mu)$ le $f_n$ convergono a $f=0$, ma è altrettanto facile vedere che non convergono quasi ovunque.
