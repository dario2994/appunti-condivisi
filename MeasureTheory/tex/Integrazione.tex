...
\begin{proposition}\label{prop:IntegraleIntegrabiliSpezzato}
	Sia $f:X\to\Rbar$ una funzione misurabile, dove $(X,\A,\mu)$ è uno spazio di misura, e siano $X_1,X_2,\cdots,X_k$ misurabili disgiunti tali che $X=\cup_{m=1}^kX_m$. Allora $f$ è integrabile su $X$ se e solo se è integrabile su $X_m$ per ogni $1\le m\le k$, e in tal caso vale che
	\begin{equation*}
		\int_X f\de\mu=\sum_{m=0}^k\int_{X_m}f\de\mu
	\end{equation*}
\end{proposition}

\begin{proof}
	Mi basta dimostrare che per una funzione $f$ misurabile e positiva $\int_X f\de\mu=\sum_{m=0}^k\int_{X_m}f\de\mu$, infatti poi la tesi segue facilmente dalla definizione di funzioni integrabili.
	
	Anche quest'ultimo fatto però è una facile conseguenza delle definizione e della \cref{prop:IntegraleSempliciSpezzato} e non ne riportiamo quindi una dimostrazione dettagliata.
\end{proof}


\begin{corollary}\label{lemma:IntegraleAMenoDiTrascurabili}
	Sia $f:X\to\Rbar$ una funzione misurabile, dove $(X,\A,\mu)$ è uno spazio di misura, e sia $N\in\A$ trascurabile. Allora $f$ è integrabile su $X$ se e solo se è integrabile su $X\setminus N$ e nel caso vale che
	\begin{equation*}
		\int_X f\de\mu=\int_{X\setminus N}f\de\mu
	\end{equation*}
\end{corollary}
\begin{proof}
	Basta considerare nella proposizione precedente $k=2$ e $X_1=X\setminus N,X_2=N$.
\end{proof}



\begin{theorem}[Integrazione per serie]
...
\begin{proof}
	...
	Se ora definiamo $f$ anche su $N$, assegnandogli dei valori arbitrari, abbiamo per il \cref{lemma:IntegraleAMenoDiTrascurabili} che $f\in \L^1(X,\A,\mu)$ e che vale
	\begin{equation*}
