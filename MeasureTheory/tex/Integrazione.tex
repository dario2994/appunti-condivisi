...
\begin{proof}
	...
	Infine da quest'ultima uguaglianza e dall'\cref{RaffinamentoIntegrale} otteniamo proprio quello che volevamo, in quanto
	\begin{equation*}
...
\begin{remark}
	...
	Infatti per la \cref{RiduzioneMisura} anche $(E,\A_E,\mu|_{\A_E})$ (come definito nella \cref{RiduzioneMisura}) è uno spazio di misura e vale facilmente che $\simp$ è misurabile, semplice e positiva anche ridotta su $(E,\A_E,\mu|_{\A_E})$.
\end{remark}
...
\begin{proof}
	...
	Dato $\epsilon>0$ sia $A_n=\{\simp_n\ge a-\epsilon\}\cap Y$. Allora abbiamo che gli $A_n$ sono misurabili, vale $A_n\subseteq A_{n+1}$, perchè le $\simp_n$ sono crescenti, e $\cup_nA_n=Y$. Inoltre $\simp_n\ge (a-\epsilon)\chi_{A_n}$ in $Y$ per la definizione stessa di $A_n$. Perciò per la \cref{prop:IntegraleSemplici} ho che
	\begin{equation*}
	...
	\end{equation*}
	dove in particolare nell'ultima uguaglianza abbiamo utilizzato la \cref{LimiteMonotonoCrescenteMisura}. Poichè in quest'ultima relazione $\epsilon$ è arbitrario otteniamo quindi
	\begin{equation*}
...
\begin{proof}
	...
	Altrettanto facilmente abbiamo anche che $\LNorm{\cdot}$ è una seminorma su $\L(X,\A,\mu)$, poichè il \cref{prop:IntegraleIntegrabili} dimostra che $\LNorm{\cdot}$ rispetta sia l'omogeneità che la disuguaglianza triangolare.
...
\begin{proof}
	Consideriamo le due successioni di funzioni misurabili positive $(g\pm f_n)$ (rispettivamente con il segno $+$ e il segno $-$). Tali funzioni sono banalmente positive in quanto $|f_n(x)|\le g(x)$, inoltre convergono puntualmente alle funzioni $g\pm f$. Per il \cref{lem:Fatou} vale che
	\begin{equation*}
