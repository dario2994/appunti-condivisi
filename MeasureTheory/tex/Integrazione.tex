% Problemi: Scegliere la notazione degli integrali, delle funzioni ``positive''. Come mi riferisco ad un item di una propositione?
...
\begin{theorem}[Beppo Levi]\label{BeppoLevi}
...
% 	La quantità $\Vert f \Vert_1$ è detta norma integrale, anche se non è effettivamente una norma.
\end{definition}
...
\begin{proof}
	...
	Dimostriamo ora che $\Vert \cdot \Vert_1$ è una seminorma su $\L^1(X,\A,\mu)$. Innanzitutto abbiamo che per la \cref{prop:IntegraleIntegrabili} vale che
	\begin{equation*}
		\Vert \lambda f \Vert_1 = \int |\lambda f |\de\mu=|\lambda| \int |f| \de\mu=|\lambda|\Vert f \Vert_1 
	\end{equation*}
	...
	\begin{equation*}
		\Vert f+g \Vert_1=\int |f+g|\de\mu\le \int (|f|+|g|)\de\mu=\int |f|\de\mu+\int |g|\de\mu=\Vert f \Vert_1+\Vert g \Vert_1
	\end{equation*}
\end{proof}
...
\begin{proof}
	Considero le due successioni di funzioni misurabili positive $(g\pm f_n)$ (rispettivamente con il segno $+$ e il segno $-$). Tali funzioni sono banalmente positive in quanto $|f_n(x)|\le g(x)$, inoltre convergono puntualmente alle funzioni $g\pm f$. Per il \cref{Fatou} vale che
	\begin{equation*}
