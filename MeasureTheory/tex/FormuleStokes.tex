...
Un'immediata del teorema appena dimostrato è la seguente:ù
...
\begin{corollary}(Teorema della divergenza)
	...
	\end{equation}
	dove l'operatore di divergenza è definito da $\text{div}u=\text{tr}\Diff u$.
\end{corollary}
...
\begin{corollary}(Prima formula di Green)
	...
	\[
		\int_{\Omega}\left(u\Delta v+<\text{grad}u,\text{grad}>\right)de x=\int_{\bar\Omega}u\frac{\partial v}{\partial\nu}\de\sigma(y)\virgola
	\]
	dove l'operatore $\Delta v = \sum_{j=1}^n \Diff_{jj} v$ è il laplaciano.
\end{corollary}
