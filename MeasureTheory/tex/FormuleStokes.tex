	...
	\begin{enumerate}
		...
		\item esistono $U$ intorno di $\bar x$ e $g$ diffeomorfismo $C^1$ da $\R^n$ in $U$ tale che\label{PRE:ii}
			\begin{itemize}
				...
				\item $U\cap \Omega = g(\R^{n-1}\times\oo{0}{\infty})$;
			\end{itemize}
	...
	\begin{description}
		...
		\item [\ImplicationProof{PRE:iii}{PRE:ii}] Se abbiamo $V\times I$ intorno di $\bar x$ e $\psi\in C^1(V,I)$ come nelle ipotesi, detto $J=\oo{-\varepsilon}{\varepsilon}$
			...
			\begin{align*}
				...
				g(V\times \oo{0}{\varepsilon})&=\{(v,\psi(v)-t)\in V\times I:t>0\}=\{(v,s)\in V\times I:s<\psi(v)\}=U\cap \Omega\punto
			\end{align*}
...
\begin{proof}
	...
	In ciascun punto $x$ del bordo, le funzioni $\psi_i$ non hanno differenziale nullo quando sono definite in $x$, per la regolarità dei punti del bordo. Esisterà quindi una direzione $e_k$ per la quale esiste almeno un $i$ tale che $\partial_k \psi_i(x)\neq 0$.
	Dato che le $\psi_i$ sono differenziabili, si ha che
	\[
		\psi_i(x+\delta e_k)=\psi_i(x)+\partial_k \psi_i(x)\cdot \delta + \smallO(\delta) = \partial_k \psi_i(x)\cdot \delta + \smallO(\delta)\punto
	\]
	Sapevamo però che, per ciascun indice $i$, $\psi_i(x+\delta e_k)<0$ solo se $x+\delta e_k\in \Omega$.
	Quindi, per $i\neq i'$, $\partial_k \psi_i(x)$ e $\partial_k \psi_{i'}(x)$ non possono essere discordi, altrimenti si ha che uno tra $\psi_i(x+\delta e_k)= \partial_k \psi_i(x) \delta + \smallO(\delta)$ e
	$\psi_i'(x+\delta e_k)=\partial_k \psi_i'(x) \delta + \smallO(\delta)$ è positivo per $\delta$ sufficientemente piccolo, ma questo sarebbe assurdo perchè in entrambi i casi $x+\delta e_k$ sta in $\Omega$.
	...
	$\partial_k \psi_i(x)$ che sono non discordi.
\end{proof}
...
\begin{proof}
	...
	\begin{description}
		...
		\item [$j\neq n$:] Possiamo assumere (a meno di riordinare le coordinate) che $j=n-1$ e, a meno di considerare un intorno più piccolo,
			...
			\[
				\int_{\Omega} \partial_{n-1} u(x) \de x = \int_W \int_J \int_I \frac{\partial}{\partial s} u(w,s,t) \de t \de s \de w\punto
			\]
			Inoltre, dato che $\frac\partial{\partial s} \int_X u(v,s)\de v = \int_X \frac\partial{\partial s} u(v,s)\de v$ e che $\frac{\partial}{\partial y} \int_a^y u(v,t)\de t=u(v,y)$,
			...
			\]
			Ora, dato che $\nu(x)_{n-1}=\nu(v,\psi(v))_{n-1}=-\partial_{n-1}\psi(v)(1+\psi(v)^2)^{-\frac{1}{2}}$, otteniamo
			\begin{align*}
