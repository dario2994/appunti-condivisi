...
\begin{theorem}\label{thm:PtRegEquiv}
	Per un punto $x\in \partial \Omega$ le seguenti tre condizioni sono equivalenti:
	\begin{enumerate}
...
\begin{proof}
	...
	\begin{description}
		\item [\ImplicationProof{PRE:i}{PRE:iii}] Dato che $\grad f(x)\neq 0$, esiste una derivata parziale non nulla, supponiamo senza perdita
			...
			che esiste un intorno $V\times I\subseteq U$ di $x$ per cui esiste $\psi\in C^1(V,I)$ tale che, per ogni $v\in V$, $f(v,\psi(v))=0$ e $\psi(v)$
			è l'unico punto $t'\in I$ tale che $f(v,t')=0$.
			Inoltre, per la continuità di $\grad f(x)$, esiste un intorno $V'\subseteq V$ di $v$ tale che $\grad f(v',\psi(v'))_n>\frac{1}{2}
			\grad f(x)_n$ per tutti i $v'\in V'$. Ora, per la differenziabilità di $f$ in $(v',\psi(v'))$ si ha, per $\delta\in \R$,
			\[
			...
			\begin{equation*}
				f(v',\psi(v')+\delta)\geq \frac{1}{2}\grad f(v',\psi(v'))_n\delta\geq \frac{1}{4}\grad f(x)_n\delta>0\punto
			\end{equation*}
		...
		\item [\ImplicationProof{PRE:iii}{PRE:ii}] Se abbiamo $V\times I$ intorno di $x$ e $\psi\in C^1(V,I)$ come nelle ipotesi, detto $J=\oo{-\varepsilon}{\varepsilon}$
...
\begin{proof}
	Per la dimostrazione si procederà in due step:
	\begin{enumerate}
	...
	\begin{align*}
		\int_{\Omega} \grad u(x)_n \de x &= \int_{V\times \cc{a}{b}} \grad u(v,t)_n \de (v,t) = \\
		& = \int_V \int_{\cc{a}{b}} \grad u(v,t)_n \de t \de v = \int_V (u(v,b) - u(v,a)) \de v = 0\punto
	\end{align*}
	...
	\begin{itemize}
		\item $U_p\cap \partial \Omega = \{(z,s):s=\psi(z)\}$,
		\item $U_p\cap \Omega = \{(z,s):s<\psi(z)\}$.
	\end{itemize}
