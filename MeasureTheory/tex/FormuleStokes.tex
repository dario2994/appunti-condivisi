...
\begin{proof}
	...
	\begin{description}
		...
		\item [$j\neq n$:] Possiamo assumere (a meno di riordinare le coordinate) che $j=n-1$ e, a meno di considerare un intorno più piccolo,
			...
			Dato che $u$ è una funzione $C^1$ definita su un compatto che contiene $W\times J\times I$, rispetta facilmente le ipotesi\footnote{L'unica ipotesi non ovvia è che la derivata parziale sia maggiorata da una funzione integrabile. Ma ciò è vero poichè la derivata parziale, essendo continua su un compatto, è maggiorata da una costante.} del \cref{thm:CommutazioneDerivataIntegrale}, quindi utilizzando le regole di composizione del differenziale (la variabile $s$ compare anche agli estremi di integrazione) otteniamo che
			\[
