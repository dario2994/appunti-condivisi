...
\begin{proof}
	...
	e, se vista come funzione da $V$ in graf$(f)$ è anche un omeomorfismo. Inoltre $Dg(x)[h]=(h,Df(x)[h])$, in particolare il differenziale è
	iniettivo in ogni punto, quindi $g$ è una immersione iniettiva, cioè è una parametrizzazione $C^1$ di graf$(f)$.
\end{proof}

\begin{remark}
	Dalla dimostrazione di questo lemma, dato che $\nabla f(x)=Df(x)$, segue immediatamente che $Dg(x)^T Dg(x)=
	(I \ \ \nabla f(x)^T)
	...
	molteplicità $1$. Tutto ciò per dire che $\det (Dg(x)^T Dg(x)) = 1+||\nabla f(x)||^2$. 
\end{remark}
