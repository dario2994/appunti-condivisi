...
\begin{proof}
	...
	\]
	ma ora abbiamo che $f_i \cdot u$ soddisfa la \cref{eq:FormulaStokes} per ogni $i$; infatti, per $i=0$ è vero per quanto appena osservato,
	...
	abbiamo che per questo intorno $U_p$ ogni funzione $C^1$ soddisfa \cref{eq:FormulaStokes}. Per linearità degli integrali e quindi dei membri che
	compaiono in questa equazione, è chiaro che le funzioni che soddisfano formano un sottospazio vettoriale di $C^1$. In particolare $u$
