...
\begin{proof}
	...
	Unendo quanto detto si ha che $\grad f_1(\bar x)\cdot v$ ha lo stesso segno di $\grad f_2(\bar x)\cdot v$ e ciò vale per ogni $v\in\R^n$. 
	Se per assurdo i gradienti non sono paralleli, scegliendo $v$ nell'ortogonale di $\grad f_1(\bar x)$ ma \emph{non} nell'ortogonale di $\grad f_2(\bar x)$ è facile notare che la condizione non è verificata e ciò implica il parallelismo dei gradienti. Per quanto riguardo il verso, basta scegliere $v=\grad f_1(\bar x)$ per ottenere che devono avere lo stesso verso.
\end{proof}
