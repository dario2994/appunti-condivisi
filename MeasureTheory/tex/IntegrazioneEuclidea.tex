...
\begin{lemma}\label{lemma:LimiteDensita} %TODO
	Fissato $\Omega\subseteq\R^n$ un aperto, sia $\varphi:\Omega\to\R^n$ una funzione differenziabile con continuità. (FORSE SERVE DIFFERENZIABILE NELLA CHIUSURA)
	Sia $\rho:\Omega\to\Rpiu$ una funzione misurabile, assicurataci dal \cref{thm:RadonNikodym} visto che $m_n\circ A$ è una misura in virtù del ??, tale che
	\begin{equation*}
		\forall A\in\M_n,\ A\subseteq \Omega:\ \int_A \rho(x)\de m_n(x)=m_n(f(A)) \punto
	\end{equation*}
	Allora la serie di funzioni
	\begin{equation*}
		f_r(x)=\frac{m_n\left(f\left(B_r(x)\right)\right)}{m_n\left(B_r(x)\right)}
	\end{equation*}
	converge in $\L$, con $r\to 0$, alla funzione $\rho$.
\end{lemma}


\begin{lemma}\label{lemma:LimiteDeterminante}
	Fissato $\Omega\subseteq\R^n$ un aperto, sia $\varphi:\Omega\to\R^n$ una funzione differenziabile.
