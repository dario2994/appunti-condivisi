...
\begin{corollary}\label{cor:CambioVariabileLineare}
	Cambio di Variabile lineare %TODO
\end{corollary}
...
\begin{lemma}\label{lemma:ContinuitaL1Palle}
	...
	Allora le funzioni $f_r$, che risultano essere continue, convergono in $\L$ alla funzione $f$ per $r$ che tende a $0$.
\end{lemma}
\begin{proof}
	...
	Ora verifichiamo, per poi poter applicare il teorema di Tonelli, che la funzione $g:\R^n\times\R^n\to\Rbar$ definita come $g(x,t)=f(x+t)-f(x)$ è misurabile nello spazio prodotto $\R^n\times\R^n$. DAFARE. %TODO
...
\begin{proof}
	...
	\end{equation*}
	e quindi, applicando proprio il \cref{lemma:ContinuitaL1Palle}, otteniamo che $\frac{m_n(\varphi(B_r(x)))}{m_n(B_r(0))}$ converge a $\rho$ in $\L$ per $r$ che tende a $0$.
