...
\begin{proof}
	...
	Fissato $c\in\R^n$, sia $f(x)=\left\lvert\chi_S(x+c)-\chi_S(x)\right\rvert$. 
	Sfruttando delle identità insiemistiche e la definizione delle funzioni caratteristiche, è facile verificare che
	\begin{equation*}
		f(x)=\chi_{T\setminus\left(T-c\right)}(x)+\chi_{\left(T-c\right)\setminus T}(x)
	\end{equation*}
	e che i due insiemi $T\setminus\left(T-c\right)$ e $\left(T-c\right)\setminus T$ hanno la stessa misura in quanto si può ottenere uno dall'altro, a meno del bordo che ha misura nulla, con una simmetria nell'origine ed una traslazione che però non cambiano la misura in virtù del \cref{prop:LebesgueProprietaIsometria}.
	
	%TODO: Calcolare la misura di uno dei due insiemi a cui si è arrivati
