...
\begin{proof}
	...
	\end{multline*}
	%TODO: Da cancellare tutta la proposizione, è stata sostituita dalla successiva.
\end{proof}
...
\begin{proof}
	...
	\end{multline}
	dove nei vari passaggi abbiamo sfruttato il fatto che l'integrale è monotono e il \cref{thm:TonelliCompleto} che si può applicare ricordando la \cref{prop:MisuraProdottoEuclidea}. Va però osservato che per poter applicare il teorema di Tonelli e quindi scambiare gli integrali come abbiamo fatto, è fondamentale che la funzione $\Omega\times B_r(0)\ni(x,h)\mapsto f(x)-f(x+h)\in\Rbar$. Questo è vero e lo lasciamo come esercizio, non banale ma neppure arduo, al lettore. %TODO Ci sono ancora parecchie piccolezze che andrebbero migliorate!
	
	Però, per la \cref{prop:ContinuitaL1}, il valore $\sup_{h\in B_r(0)} \LNorm{f(\cdot)-f(\cdot+h)}$ tende a $0$ per $r\to 0$ e applicando questo nell'\cref{eq:ContinuitaPalleDis} si ricava facilmente la tesi.
\end{proof}
