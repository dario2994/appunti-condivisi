...
% Ovunque, scriveremo implicitamente $\L$ per intendere $\L(\R^n,\M_n,m_n)$.
...
\begin{proposition}\label{prop:ContinuitaL1}
...
\begin{proof}
	...
	Dimostreremo che le funzioni indicatrici di $\S_n$ appartengono ad $\mathcal F$, poi che $\mathcal F$ è chiuso per convergenza monotona ed è uno spazio vettoriale ed infine che se due funzioni coincidono quasi ovunque ed una appartiene ad $\mathcal F$ allora anche l'altra vi appartiene. 
	L'unione di questi fatti sarà sufficiente a mostrare che $\mathcal F=\L(\R^n,\M_n,m_n)$ dimostrando così la tesi.
	
	%TODO: Finire la dimostrazione della continuità L1 seguendo lo schema preannunciato.
\end{proof}
...
\begin{theorem}\label{thm:RadonNikodym} [Radon-Nikodym]
	...
	\end{equation*}
\end{theorem} %TODO: Scrivere perchè lo lasciamo indimostrato.

\begin{definition}\label{def:MediaIntegrale} [Media integrale]
...
\begin{proof}
	...
	Ora verifichiamo, per poi poter applicare il teorema di Tonelli, che la funzione $g:\R^n\times\R^n\to\Rbar$ definita come $g(x,t)=f(x+t)-f(x)$ è misurabile nello spazio prodotto $\R^n\times\R^n$. DAFARE. %TODO: Misurabilità nel prodotto
