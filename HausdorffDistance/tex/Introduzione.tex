...
\begin{remark}
	$d_A({}\cdot{})$ è effettivamente una funzione a valori in $[0,+\infty)$ perchè vale $\inf_{a\in A}d(a,x)<\infty$, poichè $A$ è non vuoto.
\end{remark}
...
\begin{proof}
	...
	D'altra parte se $r>\delta_A(B)$, per ogni $b\in B$ vale che $r>d_A(b)$, quindi $B\subseteq U_r(A)$ e perciò $r\ge r_0$. Passando all'estremo inferiore per $r>\delta_A(B)$ ottengo quindi
	\begin{equation}\label{delta>delta'}
		\delta_A(B)=\inf\{r:r>\delta_A(B)\}\ge r_0
	\end{equation}
	
	Unendo la \cref{delta'>delta} e la \cref{delta>delta'} ottengo quindi $\delta_A(B)=r_0$, che è quello che volevo dimostrare.
\end{proof}
...
\begin{example}
	...
	\begin{center}
		...
		IMMAGINE\\
		IMMAGINE
% 		\includegraphics[width=0.7\textwidth]{EsempioHausdorff.png}
	\end{center}
	\end{figure}
