...
\begin{proof}
	...
	\end{split}
	\end{equation*}
	Quindi per $x$ abbastanza grande esiste $K$ tale che $|h_x(s)-1|<K\frac{|s|}{\sqrt{x}}$ per ogni $s\in [a,b]$.
	In particolare $|h_x(s)-1|<K\frac{M}{\sqrt{x}}$, dove $M=\max_{s\in[a,b]}\{|s|\}$.
	Perciò banalmente, per ogni $\varepsilon$ esiste $x_\varepsilon$ tale che $|h_x(s)-1|<\varepsilon$ per ogni
	$s\in[a,b]$, che equivale proprio a dire che $h_x(s)\to 1$ uniformemente su $[a,b]$ per $x\to\infty$.
